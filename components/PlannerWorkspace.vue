...
onMounted(async () => {
  ...
  plannerHealth.value = healthResult.status === "fulfilled" ? healthResult.value : null;
  if (planResult.status === "fulfilled" && planResult.value?.result?.plan) {
    ...
    confirmed.value = ["active", "paused"].includes(planResult.value.result.plan.status);
    activeView.value = "plan";
    showToast("已自动恢复最近一次学习规划");
  } else if (planResult.status === "rejected") {
    showToast(planResult.reason instanceof Error ? planResult.reason.message : "最近规划读取失败");
  }
});
...
function restorePlanningContext(envelope: AgentEnvelope) {
  ...
  const candidateSubject = restoredPlan.generation_basis?.goal_subject
  ...
  if (academic) {
    ...
    if (academic.selected_subjects?.length) form.selectedSubjects = academic.selected_subjects;
  }
  const currentScore = Number(restoredPlan.generation_basis?.goal_current_value);
  ...
  form.deadline = restoredPlan.generation_basis?.goal_deadline || restoredPlan.plan_end;
...
function navigate(view: View) {
  ...
  if (!["workspace", "tutor", "diagnosis", "records", "classroom"].includes(view) && !plan.value) {
...
function movePlannerStep(step: number) {
  ...
  if (step === 3 && (form.currentScore < 0 || form.targetScore <= form.currentScore)) {
...
function recordDiagnosticAnswer(): boolean {
  ...
  diagnosticAnswers.value = [
    ...diagnosticAnswers.value.filter((item) => item.question_id !== question.question_id),
    {
  ...
  <div class="app-shell" :class="{ 'sidebar-collapsed': sidebarCollapsed }">
    ...
    <aside class="app-sidebar" :class="{ open: sidebarOpen }">
      <div class="workspace-brand">
        ...
        <div><strong>知途 AI</strong><small>三智能体学习中心</small></div>
      ...
      <nav class="workspace-nav">
        ...
        <button v-for="item in navItems" :key="item.id" :class="{ active: activeView === item.id }" @click="navigate(item.id)">
          ...
          <i v-if="!['workspace', 'tutor', 'diagnosis', 'records', 'classroom'].includes(item.id) && !plan" />
        </button>
      </nav>
    ...
    <main class="app-main">
      <header class="topbar">
        ...
        <span class="service-state"><i /> {{ plannerHealth?.planner_generation_mode === "llm" ? `规划模型在线 · ${plannerHealth.llm_model}` : "规划模型未连接" }}</span>
      </header>
        ...
        <template v-if="activeView === 'workspace'">
          ...
          <nav class="planner-stepper" aria-label="个性化规划步骤">
            <div
              ...
              :key="item.id"
              :class="{ active: plannerStep === item.id, done: plannerStep > item.id || (item.id === 3 && diagnosticResult) }"
            >
              <span><Check v-if="plannerStep > item.id || (item.id === 3 && diagnosticResult)" :size="15" /><b v-else>{{ item.id }}</b></span>
              <div><strong>{{ item.label }}</strong><small>{{ item.note }}</small></div>
            </div>
          </nav>

          <div v-if="error" class="message error wizard-message"><CircleAlert :size="17" />{{ error }}</div>

          <section v-if="plannerStep === 1" class="planner-card planner-step-page">
            ...
            <div class="workflow-actions"><span>第 1 步，共 4 步</span><button class="primary-button" type="button" @click="movePlannerStep(2)">下一步：设置目标 <ChevronRight :size="18" /></button></div>
          </section>

          <section v-else-if="plannerStep === 2" class="planner-card planner-step-page">
            <div class="section-heading"><div><span>02</span><div><h2>填写学习目标</h2><p>这里只填写可核验的成绩目标，掌握程度将由快速诊断判断。</p></div></div><Target :size="23" /></div>
            ...
            <div class="objective-note"><Target :size="21" /><div><strong>目标用于确定训练强度，不等同于当前掌握度</strong><p>下一步的 10 题诊断会提供客观学情证据，系统不会要求你猜测自己的基础和综合应用水平。</p></div></div>
            <div class="workflow-actions"><button class="workflow-back" type="button" @click="movePlannerStep(1)">返回上一步</button><button class="primary-button" type="button" @click="movePlannerStep(3)">下一步：快速诊断 <ChevronRight :size="18" /></button></div>
          </section>

          <section v-else-if="plannerStep === 3" class="planner-card diagnostic-card planner-step-page">
            <div class="section-heading">
              <div><span>03</span><div><h2>10 题快速诊断</h2><p>直接通过客观作答估计基础、应用和迁移能力。</p></div></div>
            ...
            <div v-else-if="diagnosticSession && currentDiagnosticQuestion && !diagnosticResult" class="diagnostic-workspace">
              <div class="diagnostic-progress"><div><strong>第 {{ diagnosticIndex + 1 }} / 10 题</strong><span>{{ currentDiagnosticQuestion.knowledge_focus }}</span></div><span><i :style="{ width: `${diagnosticPercent}%` }" /></span></div>
              <article class="diagnostic-question">
                <small>{{ currentDiagnosticQuestion.dimension }} · 难度 {{ Math.round(currentDiagnosticQuestion.difficulty * 100) }}%</small>
                <h3>{{ currentDiagnosticQuestion.prompt }}</h3>
                <div class="diagnostic-options"><button v-for="(option, optionIndex) in currentDiagnosticQuestion.options" :key="optionIndex" type="button" :class="{ selected: diagnosticSelection === optionIndex }" @click="selectDiagnosticOption(optionIndex)"><b>{{ String.fromCharCode(65 + optionIndex) }}</b><span>{{ option }}</span><i v-if="diagnosticSelection === optionIndex"><Check :size="15" /></i></button></div>
              </article>
              <div class="diagnostic-confidence">
                <div><strong>你对刚才选择的答案有多确定？</strong><small>{{ diagnosticSelection === null ? '请先选择上方 A、B、C 或 D' : diagnosticIndex < 9 ? '点击后自动进入下一题' : '点击后自动提交诊断' }}</small></div>
                <div class="confidence-buttons">
                  <button
                    ...
                    type="button"
                    :class="item.tone"
                    :disabled="diagnosticSelection === null || diagnosticSubmitting"
          ...
          <section v-else class="planner-card planner-step-page">
            <div class="section-heading"><div><span>04</span><div><h2>设置可持续时间</h2><p>系统会保留机动缓冲，不会把每一分钟排满。</p></div></div><Clock3 :size="23" /></div>
            ...
            <div class="planner-actions"><button class="workflow-back" type="button" @click="movePlannerStep(3)">返回上一步</button><div><CheckCircle2 :size="20" /><span><strong>客观诊断与时间资料已准备</strong><small>{{ province.name }} · {{ planningSubjectLabel }} {{ form.currentScore }} → {{ form.targetScore }} 分</small></span></div><button class="primary-button" :disabled="loading || !selectionValid || !diagnosticResult" @click="generatePlan"><LoaderCircle v-if="loading" class="spin" :size="19" /><Sparkles v-else :size="19" />{{ loading ? 'Agent 正在规划' : '生成个性化学习计划' }}</button></div>
          </section>
        </template>
