...
import {
  createExamDiagnosticSession,
  fetchExamDiagnosticCatalog,
  gradeExamConstructedResponse,
  submitExamDiagnostic,
} from "@/lib/exam-diagnosis-client";
import { fetchStudentClassroomPortal } from "@/lib/teacher-client";
import type { ClassroomExamAssignment } from "@/lib/teacher-client";
import type {
  ExamConstructedGrade,
  ExamDiagnosticCatalog,
  ExamDiagnosticPaper,
  ExamDiagnosticResult,
  ExamDiagnosticSession,
  StudentLoginProfile,
  SubjectKey,
} from "@/lib/types";
...
const answeredCount = computed(
  () => Object.keys(choices.value).length + Object.keys(grades.value).length,
);
const selectedAssignment = computed(() =>
  teacherAssignments.value.find(
    (item) => item.assignment_id === selectedAssignmentId.value,
  ),
);
const canSubmit = computed(() => {
  ...
  return paper.value.questions.every((question) =>
    question.type === "multiple_choice"
      ? Boolean(choices.value[question.question_id])
      : Boolean(grades.value[question.question_id]),
  );
});
const diagnosisState = computed(
  () => result.value?.learning_diagnosis?.result.learning_state || null,
);
...
const pagedKnowledgeStatistics = computed(() => {
  ...
  return (learningRecord.value?.knowledge_statistics || []).slice(
    start,
    start + KNOWLEDGE_PAGE_SIZE,
  );
});
const currentElapsedSeconds = computed(() => {
  ...
  return (
    stored +
    Math.max(0, Math.round((clockNow.value - questionStartedAt.value) / 1000))
  );
});
...
onMounted(async () => {
  ...
  try {
    const [loadedCatalog, portal] = await Promise.all([
      fetchExamDiagnosticCatalog(),
      fetchStudentClassroomPortal(),
    ]);
    catalog.value = loadedCatalog;
    teacherAssignments.value = portal.exam_assignments;
    const requestedAssignment = teacherAssignments.value.find(
      (item) => item.assignment_id === props.initialAssignmentId,
    );
    if (requestedAssignment) {
      ...
      const group = loadedCatalog.subjects.find((item) =>
        item.papers.some(
          (paperItem) => paperItem.paper_id === requestedAssignment.paper_id,
        ),
      );
      if (group) selectedSubject.value = group.subject;
    } else {
      ...
      if (assignedGroup) {
        ...
        if (!catalogSubject.value)
          selectedSubject.value = loadedCatalog.subjects[0].subject;
        selectedPaperId.value = catalogSubject.value?.papers[0]?.paper_id || "";
      }
    }
  } catch (cause) {
    error.value =
      cause instanceof Error ? cause.message : "真题诊断卷目录加载失败";
  } finally {
    loading.value = false;
  }
});
...
function selectTeacherAssignment(item: ClassroomExamAssignment) {
  selectedAssignmentId.value = item.assignment_id;
  selectedPaperId.value = item.paper_id;
  const group = catalog.value?.subjects.find((subject) =>
    subject.papers.some((paperItem) => paperItem.paper_id === item.paper_id),
  );
  if (group) selectedSubject.value = group.subject;
}
...
async function startPaper() {
  ...
  error.value = "";
  try {
    const created = await createExamDiagnosticSession(
      props.profile,
      selectedPaperId.value,
...
function commitCurrentTime(): number {
  ...
  const elapsed = timingPaused.value
    ? 0
    : Math.max(0, Math.round((Date.now() - questionStartedAt.value) / 1000));
  const accumulated = Math.min(
    14_400,
    (questionDurations.value[question.question_id] || 0) + elapsed,
  );
...
function goTo(index: number) {
  ...
  currentIndex.value = Math.max(
    0,
    Math.min(index, paper.value.questions.length - 1),
  );
  questionStartedAt.value = Date.now();
...
function chooseImages(event: Event) {
  ...
  previews.value[question.question_id] = selected.map((file) =>
    URL.createObjectURL(file),
  );
  grades.value = Object.fromEntries(
    Object.entries(grades.value).filter(([id]) => id !== question.question_id),
  );
}

async function gradeCurrent() {
  const question = currentQuestion.value;
  if (!question || !session.value || !files.value[question.question_id]?.length)
    return;
  ...
  try {
    const response = await gradeExamConstructedResponse(
      ...
      files.value[question.question_id],
      durationSeconds,
    );
    ...
    error.value =
      cause instanceof Error ? cause.message : "主观题多模态评分失败";
  } finally {
    gradingId.value = "";
    timingPaused.value = false;
    questionStartedAt.value = Date.now();
      ...
      paper.value.questions
        ...
        .map((question) => ({
          ...
          duration_seconds: Math.max(
            1,
            questionDurations.value[question.question_id] || 0,
          ),
        })),
      Object.fromEntries(
...
function handleAssetError(event: Event) {
  ...
  if (
    ...
    !image.classList.contains("exam-inline-image")
  )
    return;
  ...
  if (!image.classList.contains("asset-load-failed"))
    assetLoadFailures.value += 1;
...
function handleAssetLoad(event: Event) {
  ...
  if (
    !(image instanceof HTMLImageElement) ||
    !image.classList.contains("asset-load-failed")
  )
    return;
...
<template>
  <div class="exam-diagnosis">
    <div v-if="error" class="exam-error">
    ...
    <template v-else-if="!paper && catalog">
      <section class="exam-hero student-module-hero">
        ...
        <aside>
          ...
          ><small>学生接口不下发标准答案<br />主观题由真实多模态模型阅卷</small>
        </aside>
      </section>
        ...
        <div class="teacher-task-grid">
          ...
          >
            ...
            <div>
              <small
                >{{ item.class_name }} ·
                {{ item.publisher_teacher_name }}老师</small
              ><strong>{{ item.title }}</strong>
              <p>
                {{
                  item.task_status === "completed"
                    ? `已完成 · ${item.score ?? "待评分"}/${item.paper_max ?? "--"} 分，可重做`
            ...
            <CheckCircle2
              v-if="selectedAssignmentId === item.assignment_id"
              :size="19"
            />
          </button>
        </div>
      ...
      <section v-if="activeDiagnosisSource === 'platform'" class="exam-picker">
        ...
        <div class="subject-tabs">
          <button
            v-for="item in catalog.subjects"
            :key="item.subject"
      ...
      <section v-if="activeDiagnosisSource === 'platform'" class="paper-picker">
        ...
        <div class="paper-grid">
          <button
            ...
            :class="{
              active:
                !selectedAssignmentId && selectedPaperId === item.paper_id,
            }"
            @click="
              selectedAssignmentId = '';
          ...
          >
            ...
            <div>
              ...
              ><small
                >{{ item.multiple_choice_count }} 道选择 ·
                {{ item.constructed_response_count }} 道拍照题</small
              ><i
                >{{ item.total_score }} 分 · {{ item.duration_minutes }} 分钟</i
              >
            </div>
            <CheckCircle2
              v-if="!selectedAssignmentId && selectedPaperId === item.paper_id"
              :size="18"
            />
          </button>
        </div>
        <div class="start-strip">
          <div>
            <LockKeyhole :size="18" /><span
              ><strong>{{
                selectedAssignment
                  ? `教师任务：${selectedAssignment.title}`
                  : "平台诊断作答说明"
              }}</strong
              ><small>{{
                selectedAssignment
                  ? `${selectedAssignment.class_name} · ${selectedAssignment.publisher_teacher_name}老师发布；提交后老师可查看答题进度与学情分析。`
                  : "选择题点击后自动进入下一题；大题请只上传本人的完整解题过程，最多 3 张清晰图片。"
              }}</small></span
            >
          </div>
          <button
            class="planning-primary-action"
            :disabled="starting || !selectedPaperId"
            @click="startPaper"
          >
            <LoaderCircle
              v-if="starting"
    ...
    <template v-else-if="paper && session && !result">
      <section class="exam-toolbar">
        <div>
          <small>{{ paper.subject_label }} · 高考真题</small
          ><strong>{{ paper.title }}</strong>
        </div>
        <span class="live-time"
          ><Clock3 :size="15" />本题
          {{ formatDuration(currentElapsedSeconds) }}</span
        ><span>{{ answeredCount }}/{{ paper.question_count }} 已完成</span
        ><button @click="reset"><RotateCcw :size="15" />重新选卷</button>
      </section>
      <div class="exam-progress">
        <i
          :style="{ width: `${(answeredCount / paper.question_count) * 100}%` }"
        />
      </div>
      <section v-if="assetLoadFailures" class="asset-warning">
        <AlertTriangle :size="18" /><span
          >有
          {{ assetLoadFailures }}
          个原题图形重试后仍未加载。请保持当前页面并刷新浏览器；作答记录和已累计用时不会因题间切换丢失。</span
        >
      </section>
      ...
      <div class="exam-layout">
        <aside class="question-nav">
          <header>
            <strong>答题卡</strong><small>蓝色已完成 · 当前题高亮</small>
          </header>
          <div>
            <button
              ...
              :class="{
                current: currentIndex === question.sequence - 1,
                done:
                  choices[question.question_id] || grades[question.question_id],
                subjective: question.type === 'constructed_response',
              }"
              @click="goTo(question.sequence - 1)"
            >
              {{ question.sequence }}
            </button>
          </div>
          <section>
            <span><i />选择题</span><span><i />拍照题</span>
          </section>
        </aside>
        ...
        <main
          ...
          @load.capture="handleAssetLoad"
        >
          <header>
            <div>
              <span>第 {{ currentQuestion.sequence }} 题</span
              ><b>{{
            ...
            <small
              >{{ currentQuestion.max_score }} 分 · 难度
              {{ Math.round(currentQuestion.difficulty * 100) }}% ·
              {{ currentQuestion.knowledge_tags.join("、") }}</small
            >
          </header>
          <div class="question-source">
            <FileCheck2 :size="15" /><span
              >{{ currentQuestion.source.source_title }} · 原题第
              {{ currentQuestion.source.original_number }} 题</span
            ><i>已校验来源</i>
          </div>
          ...
          >
            <button
              ...
              :key="option.key"
              :class="{
                selected: choices[currentQuestion.question_id] === option.key,
              }"
              @click="selectOption(option.key)"
            >
              <b>{{ option.key }}</b
              ><span v-html="option.content_html" /><CheckCircle2
                v-if="choices[currentQuestion.question_id] === option.key"
                :size="19"
              />
            </button>
          </div>
          ...
          <section v-else class="photo-answer">
            <div class="photo-guide">
              <Camera :size="22" /><span
                ><strong>请拍照上传你的完整解题过程</strong
                ><small
                  >保留推导、单位、图形和最终结论；模型只按图片中可核验的内容评分，不会脑补步骤。</small
                ></span
              >
            </div>
            <label class="upload-zone"
              ...
              /><UploadCloud :size="28" /><strong>{{
                files[currentQuestion.question_id]?.length
                  ? `已选择 ${files[currentQuestion.question_id].length} 张图片`
                  : "点击选择或拍摄作答图片"
              }}</strong
              ><small
                >JPG / PNG / WebP，单张不超过 10MB，最多 3 张</small
              ></label
            >
            <div
              v-if="previews[currentQuestion.question_id]?.length"
              class="image-previews"
            >
              <img
                v-for="url in previews[currentQuestion.question_id]"
                :key="url"
                :src="url"
                alt="学生作答预览"
              />
            ...
            <button
              class="grade-photo"
              :disabled="
                gradingId === currentQuestion.question_id ||
                !files[currentQuestion.question_id]?.length
              "
              @click="gradeCurrent"
            >
              <LoaderCircle
                v-if="gradingId === currentQuestion.question_id"
                class="spin"
                :size="18"
              /><Sparkles v-else :size="18" />{{
                gradingId === currentQuestion.question_id
                  ? "真实多模态模型正在识别与评分"
                  : grades[currentQuestion.question_id]
                    ? "重新识别并评分"
                    : "提交本题给模型评分"
              }}
            </button>
            <article
              v-if="grades[currentQuestion.question_id]"
              class="grade-feedback"
              :class="{
                review:
                  grades[currentQuestion.question_id].requires_manual_review,
              }"
            >
              <div>
                <CheckCircle2 :size="20" /><strong
                  >{{ grades[currentQuestion.question_id].score ?? "待复核" }} /
                  {{ grades[currentQuestion.question_id].max_score }} 分</strong
                ><span
                  >置信度
                  {{
                    Math.round(
                      grades[currentQuestion.question_id].confidence * 100,
                    )
                  }}%</span
                >
              </div>
              <p>{{ grades[currentQuestion.question_id].feedback }}</p>
              <small v-if="grades[currentQuestion.question_id].review_reason">{{
                grades[currentQuestion.question_id].review_reason
              }}</small>
            </article>
          </section>
          ...
          <footer>
            ...
            >
              <ArrowLeft :size="16" />上一题</button
            ><button
              v-if="currentIndex < paper.questions.length - 1"
              @click="goTo(currentIndex + 1)"
            >
            ...
            ><button
              ...
              @click="submitPaper"
            >
              <LoaderCircle v-if="submitting" class="spin" :size="17" /><Send
                v-else
                :size="17"
              />{{
                canSubmit
                  ? "提交整套诊断卷"
                  : `还需完成 ${paper.question_count - answeredCount} 题`
              }}
            </button>
          </footer>
        </main>
    ...
    <template v-else-if="result && paper">
      <section class="result-hero">
        <div>
          <span><BookOpenCheck :size="18" /> 真题诊断已完成</span>
          <h2>
            {{ result.score }} <small>/ {{ result.paper_max }} 分</small>
          </h2>
          <p>
            选择题由标准答案精确判分；主观题由多模态模型按原卷答案逐项评分。
          </p>
        </div>
        <aside>
          <strong
            >{{
              result.objective_results.filter((item) => item.is_correct).length
            }}/{{ result.objective_results.length }}</strong
          ><small>选择题正确</small
          ><b
            >{{ result.constructed_results.length }}/{{
              paper.constructed_response_count
            }}</b
          ><small>主观题已阅</small>
        </aside>
      </section>
      <section
        v-if="result.manual_review_question_ids.length"
        class="review-warning"
      >
        <AlertTriangle :size="20" /><span
          ><strong
            >有
            {{ result.manual_review_question_ids.length }}
            道题需要人工复核</strong
          ><small
            >系统没有用低置信度猜测冒充确定分，当前总分请视为待复核成绩。</small
          ></span
        >
      </section>
      <section v-if="learningRecord" class="record-overview">
        ...
        <div class="record-metrics">
          <article>
            ...
            <div>
              <strong>{{
                formatDuration(learningRecord.total_duration_seconds)
              }}</strong
              ><small>整卷有效作答用时</small>
            </div>
          </article>
          <article>
            <Target :size="21" />
            <div>
              <strong
                >{{
                  Math.round(learningRecord.objective_accuracy * 100)
                }}%</strong
              ><small>选择题准确率</small>
            </div>
          </article>
          <article>
            ...
            <div>
              <strong
                >{{ Math.round(learningRecord.score_accuracy * 100) }}%</strong
              ><small>{{
                learningRecord.is_provisional
                  ? "当前得分率（待复核）"
                  : "整卷得分率"
              }}</small>
            </div>
          ...
          <article>
            <ShieldCheck :size="21" />
            <div>
              <strong>{{ learningRecord.knowledge_statistics.length }}</strong
              ><small>已统计知识点</small>
            </div>
          </article>
        </div>
        <div class="knowledge-record-table">
          <div class="record-row record-head">
            ...
            ><span>平均每题</span><span>准确率 / 得分率</span>
          </div>
          <div
            v-for="item in pagedKnowledgeStatistics"
          ...
          >
            ...
            ><span>{{ formatDuration(item.average_duration_seconds) }}</span
            ><b
              :class="{ weak: item.accuracy !== null && item.accuracy < 0.6 }"
              >{{
                item.accuracy === null
                  ? "待评分"
        ...
        <PaginationControls
          :page="knowledgePage"
          :total="learningRecord.knowledge_statistics.length"
          :page-size="KNOWLEDGE_PAGE_SIZE"
          label="个知识点"
      ...
      <div class="result-grid">
        <section>
          ...
          <div class="score-list">
            <article
              v-for="question in pagedResultQuestions"
              :key="question.question_id"
            >
              ...
              <div>
                ...
                ><small
                  ...
                  · 原题 {{ question.source.original_number }} · 用时
                  {{
                    formatDuration(
                      learningRecord?.question_records.find(
              ...
              <b
                >{{
                  result.objective_results.find(
                    (item) => item.question_id === question.question_id,
                  )?.score ??
                  result.constructed_results.find(
                    (item) => item.question_id === question.question_id,
                  )?.score ??
                  "复核"
                }}/{{ question.max_score }}</b
              >
            </article>
          </div>
          <PaginationControls
        ...
        <section>
          ...
          <div v-if="diagnosisState" class="diagnosis-copy">
            ...
            <div>
              <strong>下一步</strong
              ><small>{{
                diagnosisState.narrative.next_evidence_request
              }}</small>
            </div>
          </div>
          <div v-else class="diagnosis-copy unavailable">
            <AlertTriangle :size="20" />
            <p>
              成绩与学习记录已保存，但学情报告未生成；没有使用固定模板替代真实模型。
            </p>
          </div>
        </section>
      </div>
      <button class="new-paper" @click="reset">
        <RotateCcw :size="17" />选择另一套真题诊断卷
      </button>
    </template>
  </div>
</template>
...
.exam-hero {
  ...
  padding: 31px 36px;
  color: #fff;
  background: linear-gradient(135deg, #0d3569, #165dcb 62%, #149985);
  border-radius: 18px;
}
.exam-hero > div {
  max-width: 720px;
...
.exam-hero aside strong {
  font-size: 11px;
}
.exam-hero aside small {
  text-align: center;
  color: #d8e6f5;
  font-size: 8px;
  line-height: 1.6;
...
.exam-picker header small,
.paper-picker header small,
.result-grid header small {
  color: #5d84c3;
  font-size: 8px;
  font-weight: 850;
}
...
.subject-tabs button.active small {
  color: #dbe9ff;
}
.paper-grid {
  display: grid;
...
.paper-grid button.active {
  border-color: #68a1f1;
  background: #f0f6ff;
  box-shadow: 0 0 0 2px #deebff;
}
.paper-grid button.active > svg {
  color: #1c9a70;
}
.start-strip {
  display: flex;
...
.new-paper {
  ...
  color: #fff;
  border: 0;
  background: linear-gradient(135deg, #155eef, #159d8c);
  border-radius: 9px;
  font-size: 9px;
  font-weight: 800;
}
...
.exam-progress i {
  display: block;
  height: 100%;
  background: linear-gradient(90deg, #155eef, #20ab98);
}
.asset-warning {
  display: flex;
  align-items: center;
...
.question-nav button.subjective {
  border-style: dashed;
}
...
.question-content :deep(.exam-shared-context) {
  display: block;
  ...
  border-left: 4px solid #4382d7;
  border-radius: 0 10px 10px 0;
}
.question-content :deep(.exam-shared-context::before) {
  content: "题目材料 · 请阅读后作答";
  display: block;
  position: sticky;
  top: -17px;
...
.question-content :deep(.exam-inline-image),
.answer-options :deep(.exam-inline-image) {
  display: inline-block;
  max-width: 100%;
  max-height: 180px;
  vertical-align: middle;
}
.question-content :deep(.exam-formula-image),
.answer-options :deep(.exam-formula-image),
.question-content :deep(.exam-inline-image[src*=".svg"]),
.answer-options :deep(.exam-inline-image[src*=".svg"]) {
  width: auto;
  height: clamp(18px, 1.6em, 26px);
...
.question-content :deep(.asset-load-failed),
.answer-options :deep(.asset-load-failed) {
  min-width: 150px;
  min-height: 38px;
  padding: 8px;
  color: #9b3e47;
  border: 1px dashed #d99ca3;
  background: #fff3f4;
  font-size: 9px;
}
.answer-options {
...
.answer-options button.selected > b {
  color: #fff;
  background: #1760d0;
}
.photo-answer {
...
.grade-feedback > div span {
  margin-left: auto;
}
.grade-feedback.review {
  color: #855f16;
  background: #fff9e9;
}
...
.question-card > footer .submit-paper {
  margin-left: auto;
  color: #fff;
  background: #1760d0;
}
.result-hero {
  ...
  padding: 28px 34px;
  color: #fff;
  background: linear-gradient(135deg, #123a72, #1766d2, #149e88);
  border-radius: 17px;
}
.result-hero h2 {
  font-size: 38px;
...
@media (max-width: 1100px) {
  ...
  .record-row {
    grid-template-columns: minmax(150px, 1.4fr) repeat(4, minmax(72px, 1fr));
  }
}
...
@media (max-width: 720px) {
  ...
  .record-row span:nth-of-type(3)::before {
    content: "平均：";
  }
  .record-row b::before {
    content: "正确率：";
...
.teacher-task-grid > button > span {
  display: grid;
  width: 39px;
  height: 39px;
  place-items: center;
  color: #fff;
  background: linear-gradient(135deg, #1760d0, #159884);
  border-radius: 10px;
}
.teacher-task-grid > button > div {
  display: grid;
  flex: 1;
  gap: 5px;
}
...
.teacher-task-grid > button.active {
  border-color: #4d8ee9;
  background: #eef6ff;
  box-shadow: 0 0 0 2px #dcecff;
}
.teacher-task-grid > button.active > svg {
  color: #168d71;
}
@media (max-width: 720px) {
  .teacher-task-grid {
