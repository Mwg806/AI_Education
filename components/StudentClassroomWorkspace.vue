...
import {
  ...
  GraduationCap, LoaderCircle, LogOut, School, UsersRound, XCircle,
} from "@lucide/vue";
import { computed, onMounted, ref } from "vue";

import PaginationControls from "@/components/PaginationControls.vue";
import {
  fetchStudentClassroomPortal, joinClassroom, requestClassroomLeave,
} from "@/lib/teacher-client";
import type { ClassroomSummary, StudentClassroomPortal } from "@/lib/teacher-client";

const emit = defineEmits<{ openDiagnosis: [paperId: string] }>();
const portal = ref<StudentClassroomPortal>({ classrooms: [], announcements: [], exam_assignments: [], leave_requests: [] });
const classCode = ref("");
...
const pagedClassrooms = computed(() =>
  portal.value.classrooms.slice((classroomPage.value - 1) * PAGE_SIZE, classroomPage.value * PAGE_SIZE),
);
const pagedAnnouncements = computed(() =>
  portal.value.announcements.slice((noticePage.value - 1) * PAGE_SIZE, noticePage.value * PAGE_SIZE),
);
const pagedAssignments = computed(() =>
  portal.value.exam_assignments.slice((examPage.value - 1) * PAGE_SIZE, examPage.value * PAGE_SIZE),
);
const pagedLeaveRequests = computed(() =>
  portal.value.leave_requests.slice((leavePage.value - 1) * PAGE_SIZE, leavePage.value * PAGE_SIZE),
);
...
async function requestLeave(classroom: ClassroomSummary) {
  ...
  if (!window.confirm(`确定申请退出“${classroom.class_name}”吗？教师同意前你仍是班级成员。`)) return;
...
function leaveStatusLabel(status: string) {
  return status === "pending" ? "等待教师审批" : status === "approved" ? "教师已同意，已退出班级" : "教师未同意退出";
}
...
<template>
  <div class="student-classroom-page">
    ...
    <template v-else>
      <section class="student-class-section"><header><div><small>MY CLASSROOMS</small><h2>已加入班级</h2></div><span>{{ portal.classrooms.length }} 个</span></header><div v-if="portal.classrooms.length" class="joined-class-grid"><article v-for="item in pagedClassrooms" :key="item.id" :class="{leaving:item.leave_request_status==='pending'}"><span><School :size="22" /></span><div><small>{{ item.school_name }}</small><h3>{{ item.class_name }}</h3><p>{{ item.teacher_name }}老师 · {{ item.subject || '综合班级' }}</p><i v-if="item.leave_request_status==='pending'">退出申请正在等待教师审批</i></div><div class="class-actions"><button class="copy-class" @click="copyCode(item.class_code)"><Copy :size="14" />{{ item.class_code }}</button><button class="leave-class" :disabled="item.leave_request_status==='pending'||requestingClassId===item.id" @click="requestLeave(item)"><LoaderCircle v-if="requestingClassId===item.id" class="spin" :size="14" /><LogOut v-else :size="14" />{{ item.leave_request_status==='pending'?'等待审批':item.leave_request_status==='rejected'?'重新申请退出':'申请退出' }}</button></div></article></div><div v-else class="classroom-empty"><GraduationCap :size="34" /><strong>尚未加入班级</strong><p>向教师获取 8 位班级码后，在上方输入即可加入。</p></div><PaginationControls :page="classroomPage" :total="portal.classrooms.length" :page-size="PAGE_SIZE" label="个班级" @change="classroomPage=$event" /></section>
      <section v-if="portal.leave_requests.length" class="student-class-section leave-progress"><header><div><small>LEAVE REQUESTS</small><h2>退出申请进度</h2></div><span>{{ portal.leave_requests.length }} 条</span></header><article v-for="item in pagedLeaveRequests" :key="item.request_id" :class="item.status"><span><LoaderCircle v-if="item.status==='pending'" :size="19" /><CheckCircle2 v-else-if="item.status==='approved'" :size="19" /><XCircle v-else :size="19" /></span><div><strong>{{ item.class_name }}</strong><p>{{ leaveStatusLabel(item.status) }}</p><small>申请于 {{ new Date(item.requested_at).toLocaleString('zh-CN') }}<template v-if="item.reviewer_note"> · 教师说明：{{ item.reviewer_note }}</template></small></div></article><PaginationControls :page="leavePage" :total="portal.leave_requests.length" :page-size="PAGE_SIZE" label="条申请" @change="leavePage=$event" /></section>
      <div class="student-class-columns">
        <section class="student-class-section notice-list"><header><div><small>ANNOUNCEMENTS</small><h2>班级通知与作业</h2></div><Bell :size="20" /></header><article v-for="item in pagedAnnouncements" :key="item.announcement_id"><span :class="item.announcement_type"><Bell :size="17" /></span><div><small>{{ item.class_name }} · {{ noticeLabel(item.announcement_type) }}</small><h3>{{ item.title }}</h3><p>{{ item.content }}</p><time><CalendarClock :size="13" />{{ item.due_at ? `截止 ${new Date(item.due_at).toLocaleString('zh-CN')}` : `发布于 ${new Date(item.created_at).toLocaleString('zh-CN')}` }}</time></div></article><div v-if="!portal.announcements.length" class="classroom-empty compact"><Bell :size="29" /><strong>暂无班级通知</strong></div><PaginationControls :page="noticePage" :total="portal.announcements.length" :page-size="PAGE_SIZE" label="条通知" @change="noticePage=$event" /></section>
        <section class="student-class-section exam-task-list"><header><div><small>DIAGNOSTIC TASKS</small><h2>教师发布的诊断卷</h2></div><ClipboardCheck :size="20" /></header><article v-for="item in pagedAssignments" :key="item.assignment_id"><span><BookOpenCheck :size="19" /></span><div><small>{{ item.class_name }} · {{ item.status==='published'?'进行中':'已关闭' }}</small><h3>{{ item.title }}</h3><p>试卷编号：{{ item.paper_id }}</p><time><CalendarClock :size="13" />{{ item.due_at ? `截止 ${new Date(item.due_at).toLocaleString('zh-CN')}` : '未设置截止时间' }}</time></div><button v-if="item.status==='published'" @click="emit('openDiagnosis', item.paper_id)">前往诊断</button></article><div v-if="!portal.exam_assignments.length" class="classroom-empty compact"><ClipboardCheck :size="29" /><strong>暂无诊断任务</strong></div><PaginationControls :page="examPage" :total="portal.exam_assignments.length" :page-size="PAGE_SIZE" label="份任务" @change="examPage=$event" /></section>
      </div>
    </template>
  </div>
</template>
...
.student-classroom-page{font-size:15px;line-height:1.55}.classroom-hero>div>span{font-size:13px}.classroom-hero p{font-size:15px}.classroom-hero form>label{font-size:14px}.classroom-hero input{height:48px;font-size:16px}.classroom-hero form button{min-width:104px;font-size:14px}.classroom-hero form small{font-size:12px}
.student-class-section>header small{font-size:11px}.student-class-section>header h2{font-size:20px}.student-class-section>header>span{font-size:13px}.joined-class-grid small{font-size:12px}.joined-class-grid h3{font-size:16px}.joined-class-grid p,.joined-class-grid button{font-size:13px}
.joined-class-grid article{align-items:flex-start;padding-top:48px}.joined-class-grid article.leaving{border-color:#e8c878;background:#fffaf0}.joined-class-grid article>div>i{color:#9a6815;font-size:12px;font-style:normal;font-weight:700}.joined-class-grid article>.class-actions{position:absolute;inset:10px 10px auto 10px;display:flex;align-items:center;justify-content:space-between;gap:8px}.class-actions button{position:static;display:inline-flex;min-height:34px;align-items:center;gap:5px;padding:0 9px;border-radius:7px}.class-actions .copy-class{color:#5e7899;border:0;background:transparent}.class-actions .leave-class{color:#9a4545;border:1px solid #efc9c9;background:#fff7f7}.class-actions .leave-class:disabled{color:#9a761f;border-color:#ead79d;background:#fff9e8}.leave-progress>article{display:flex;align-items:flex-start;gap:12px;padding:15px 2px;border-bottom:1px solid #edf1f7}.leave-progress>article>span{display:grid;width:39px;height:39px;flex:0 0 auto;place-items:center;border-radius:10px}.leave-progress>article.pending>span{color:#9a6b16;background:#fff1c9}.leave-progress>article.approved>span{color:#167159;background:#def4eb}.leave-progress>article.rejected>span{color:#a14b4b;background:#fce9e9}.leave-progress article>div{display:grid;gap:4px}.leave-progress strong{color:#294360;font-size:15px}.leave-progress p{margin:0;color:#637791;font-size:14px}.leave-progress small{color:#8a99aa;font-size:12px}
.notice-list small,.exam-task-list small{font-size:12px}.notice-list h3,.exam-task-list h3{font-size:16px}.notice-list p,.exam-task-list p{font-size:14px}.notice-list time,.exam-task-list time{font-size:12px}.exam-task-list article>button{min-height:42px;font-size:14px}.classroom-empty strong{font-size:15px}.classroom-empty p,.classroom-message,.classroom-loading{font-size:13px}
@media(max-width:1120px){.student-class-columns{grid-template-columns:1fr}.joined-class-grid{grid-template-columns:1fr 1fr}}@media(max-width:700px){.joined-class-grid{grid-template-columns:1fr}}
</style>
