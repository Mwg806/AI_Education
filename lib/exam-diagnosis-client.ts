...
function normalizeAssetHtml(html: string): string {
  return html
    .replaceAll(
      'src="/agent-api/api/v1/exam-diagnostics/assets/',
      `src="${API_BASE}${ASSET_API_PATH}`,
    )
    .replaceAll(
    ...
    .replace(
      /src="([^"]+\/exam-diagnostics\/assets\/[^"]+)"/g,
      'src="$1?asset_version=20260802f"',
    );
}
...
async function parseResponse<T>(response: Response): Promise<T> {
  ...
  if (!response.ok) {
    throw new Error(
      data.errors?.[0]?.message || data.detail || "高考真题诊断服务请求失败",
    );
  }
  return data;
}
...
export async function createExamDiagnosticSession(
  ...
  const payload = await parseResponse<{
    session: ExamDiagnosticSession;
    paper: ExamDiagnosticPaper;
  }>(response);
...
export async function gradeExamConstructedResponse(
  ...
  form.append(
    "duration_seconds",
    String(Math.max(1, Math.min(14_400, Math.round(durationSeconds)))),
  );
