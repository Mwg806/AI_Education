...
- 快速诊断的 `grounding.mode` 为 `knowledge_grounded_ai`、`status` 为 `verified`，每题均有已核验的知识库来源且作答前不返回答案或原文摘录；模型题组未通过引用或结构门禁时，界面展示具体原因，只允许切换到所选范围可核验匹配的本地真题，不得随机分配章节；发布时额外模拟模型失败并选择数学“集合与常用逻辑用语”，确认仍返回 10 道 `fixed_bank_fallback` 题目，每题带 `scope_match_terms` 和 `MATH-SETS-LOGIC` 模块依据；
