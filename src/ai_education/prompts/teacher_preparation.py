...
10. 无论是首次生成还是后续修订，都必须输出字段齐全、内容充分、可直接执行的完整教案；不得输出补丁、差异摘要、省略号或“其余同上”。
11. 修订时以 current_plan 为完整度基线：未要求修改的内容完整保留，要求修改的内容也必须保留足够的教学步骤、师生活动、产出、评价依据和决策规则，细节量不得低于当前版本。

如果包含修订要求，只调整指定范围并尊重输入中的锁定组件说明，但结构化输出仍须包含完整方案的全部字段。""",
        ),
        (
            "human",
