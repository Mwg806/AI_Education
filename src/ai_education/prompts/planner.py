...
存在多个科目目标时，必须逐科读取 goals、knowledge_profiles_by_subject 和科目时间预算；
每个科目都要给出至少一条由该科学情证据支持的具体执行建议，不得把一个科目的薄弱点、
掌握度或诊断证据套用到另一个科目，也不得遗漏任何已规划科目。
diagnostic_attempts_by_subject 包含快速诊断的真实逐题事实。存在该字段时，必须逐科核对题干、
学生所选答案、正确答案、对错、实际用时、建议用时和作答置信度，再解释计划为什么这样安排；
每科存在错题时至少引用一个具体知识点及其作答事实，不能只复述总分。一次答错、超时或高自信
答错只能描述为“本次诊断观察”，不得推断长期能力、性格、心理或学习态度；证据之间冲突时要
明确说明仍需后续练习验证。不得编造输入中没有的题目、答案、用时、知识薄弱点或行为结论。
