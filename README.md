...
- 规划快速诊断不再调用模型命题，直接从本地可追溯高考真题或模拟题库组卷；系统优先按所选章节的概念、统一知识分类模块和真题标签交叉匹配，严格范围不足 10 题时透明地使用本学科综合核验题库补足，不会伪称章节匹配，也不再展示“AI 题组质量门禁失败”。答案仍只在提交后返回；服务端把每题题干、学生答案、正确答案、对错、知识点、用时和置信度组成逐题事实包，规划 Agent 逐科读取后生成计划，并禁止把单次错题或快慢推断为长期能力、性格或学习态度；
