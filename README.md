...
- 学情诊断 Agent：融合多次独立测评证据，输出带置信度、来源和版本的知识状态、稳定错因、叙事解释与教师审核流；导入题目/解法照片时结合有效尺寸、总像素、对比度与 OCR 可识别度判断图片质量，不再把清晰的窄幅步骤截图仅因短边不足 700 像素误报为低分辨率，并按文件展示实际尺寸和核对原因；高考真题诊断卷会额外生成纯中文的本卷水平、细分薄弱/优势知识点、逐题用时与作答状态分析，并明确区分本次行为和跨测次稳定结论，不用单卷推断性格或长期学习态度；
