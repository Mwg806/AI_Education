...
- 规划快速诊断采用知识库约束命题：先从本地 `Knowledge/91_indexes/knowledge.db` 检索所选章节对应的课程标准、知识分类和评价依据，再要求模型按固定十题槽位逐题引用原文；服务端校验章节、片段 ID、逐字摘录和五维各两题结构，不合格最多完整重试 3 次。只有所选范围可核验匹配的本地高考真题才能作为兜底；兜底匹配会先清理教材分册/章序噪声，把复合章名拆为有效概念，并与统一知识分类模块和真题知识标签交叉核验，避免因长标题机械截断而把已有相关真题误判为 0 道。页面展示质量门禁原因、题目来源和实际章节匹配词，不再把所有失败笼统描述为“AI 暂时不可用”；
