...
- 规划快速诊断不再调用模型命题，直接从可追溯高考真题、模拟题和教材配套练习组卷；生产环境优先从私有 OSS 读取离线构建、按学科压缩并带 SHA-256 清单的结构化题库，ECS 只保存小型校验缓存，不同步 39GB 原始资料。OSS 暂时不可用或某科尚无补充分片时自动回退本地高考真题库。系统优先按所选章节的概念、统一知识分类模块和题目标签交叉匹配，严格范围不足 10 题时透明地使用本学科综合核验题库补足，不会伪称章节匹配，也不再展示“AI 题组质量门禁失败”。答案仍只在提交后返回；服务端把每题题干、学生答案、正确答案、对错、知识点、用时和置信度组成逐题事实包，规划 Agent 逐科读取后生成计划，并禁止把单次错题或快慢推断为长期能力、性格或学习态度；
